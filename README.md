# 🌟 Tushar Rathore - Portfolio

> **Full Stack Developer specializing in React.js, Next.js, Node.js, Express.js, and SQL**

[![Portfolio](https://img.shields.io/badge/Portfolio-Live-brightgreen?style=for-the-badge&logo=vercel)](https://portfolio-tushh-r.vercel.app)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/tushar-rathore-9927761b3/)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-black?style=for-the-badge&logo=github)](https://github.com/tushh-codes)
[![LeetCode](https://img.shields.io/badge/LeetCode-Solve-orange?style=for-the-badge&logo=leetcode)](https://leetcode.com/tushh_r/)

## 👨‍💻 About Me

Full Stack Developer with **2+ years of experience** specializing in building scalable web applications using React.js, Next.js, Node.js, Express.js, and SQL. Skilled in designing robust front-end interfaces and efficient back-end APIs, integrating databases, and optimizing performance for seamless user experiences across platforms.

- 🔭 Currently working at **Webanix Solutions** as Full Stack Developer
- 🌱 Specialized in **React.js, Next.js, Node.js, Express.js, TypeScript, SQL**
- 💼 Built enterprise systems used by **5k+ employees** and external clients
- 📈 Developed full-stack applications with **front-end and back-end** integration
- 📍 Based in **Udaipur, Rajasthan, India**

## 🛠️ Tech Stack

### Frontend

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### Backend & Databases

![Node.js](https://img.shields.io/badge/Node.js-43853D?style=for-the-badge&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-404D59?style=for-the-badge&logo=express&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white)

### Tools & Technologies

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-0078D4?style=for-the-badge&logo=visual%20studio%20code&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)

## 🚀 Featured Projects

### 🏢 HRMS Management System

**Enterprise HR Platform for 5k+ employees**

- Complete employee lifecycle management with onboarding, payroll, and leave management
- Advanced reporting and analytics with role-based dashboards
- **Tech:** Next.js, Node.js, TypeScript, Tailwind CSS, Ant Design, SQL
- **Impact:** Reduced HR operational workload by 50%
- [🔗 Live Demo](http://115.241.153.220:10000/login)

### 🏆 Bharatatma Puraskar

**Award Application System**

- Two-stage form submission system with admin control panel
- PDF & Excel export functionality with advanced search features
- **Tech:** Next.js, TypeScript, Tailwind CSS, Ant Design
- [🔗 Live Demo](https://forms.bharatatmapuraskar.org/)

### 🏭 Tempsens Production Management

**SAP-Integrated Manufacturing System**

- Real-time production tracking with interactive dashboards
- Stage management and overdue job alerts
- **Tech:** React.js, Tailwind CSS, JavaScript, SAP Integration
- **Impact:** Improved delivery compliance by 30%
- [🔗 Live Demo](https://tempsensfrontend.paperscissorstone.in/)

### 📸 Photography Salon Tracker

**Portfolio Management Platform**

- Track salon participation, submissions, and awards
- Personalized reports and achievement dashboards
- **Tech:** Next.js, TypeScript, Tailwind CSS
- **Impact:** Reduced manual tracking by 50%
- [🔗 Live Demo](https://salonapp.curatedlearn.in/)

### 🍽️ FCM - Food Court Management

**Employee Meal Ordering System**

- QR code-based wallet payment system with subscription plans
- Admin dashboard with comprehensive sales reports
- **Tech:** React.js, React Hook Form, TypeScript, Tailwind CSS
- **Impact:** Reduced order processing time by 40%

### 🏗️ Stone Warehouse

**Dynamic Warehouse Website**

- Content management portal with real-time inventory display
- Animated UI for enhanced user engagement
- **Tech:** Next.js, TypeScript, Node.js, Tailwind CSS

## 💼 Professional Experience

### **Full Stack Developer** - Webanix Solutions _(Dec 2023 - Present)_

- Developed and maintained full-stack web applications using Next.js, Node.js, Express.js, and SQL
- Delivered enterprise-grade systems including HRMS, CMS-based platforms, and management dashboards
- Integrated secure authentication, optimized REST APIs, and deployed applications on cPanel

### **Front End Developer Intern** - Webanix Solutions _(Oct 2023 - Dec 2023)_

- Collaborated in UI/UX development and API integrations using React.js and Next.js
- Improved code modularity and delivery timelines by 25%

### **Web Developer Intern** - Zeetron Networks _(Jul 2022 - Dec 2022)_

- Contributed to full-stack web development projects
- Enhanced application performance and delivery timelines

## 🎓 Education

**B.Tech in Computer Science and Engineering**  
_Bikaner Technical University_ | Aug 2019 - Jul 2023  
**CGPA:** 8.9

## 📊 GitHub Stats

![Tushar's GitHub stats](https://github-readme-stats.vercel.app/api?username=tushh-codes&show_icons=true&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=tushh-codes&layout=compact&theme=radical)

## 🤝 Let's Connect!

I'm always open to exciting opportunities and collaborations!

- 💼 **LinkedIn:** [tushar-rathore-9927761b3](https://www.linkedin.com/in/tushar-rathore-9927761b3/)
- 📧 **Email:** [tushar2643@gmail.com](mailto:tushar2643@gmail.com)
- 💻 **GitHub:** [tushh-codes](https://github.com/tushh-codes)
- 🧩 **LeetCode:** [tushh_r](https://leetcode.com/tushh_r/)
- 📍 **Location:** Udaipur, Rajasthan, India

## 🚀 Getting Started

This portfolio is built with **Next.js** and **TypeScript**. To run it locally:

```bash
# Clone the repository
git clone https://github.com/tushh-codes/portfolio.git

# Navigate to the project directory
cd portfolio

# Install dependencies
npm install

# Start the development server
npm run dev
```

Open [http://localhost:3000](http://localhost:3000) to view the portfolio in your browser.

## 📦 Project Structure

```
├── src/
│   ├── app/                 # Next.js app directory
│   ├── components/          # Reusable components
│   ├── data/               # Data files (projects, experience, skills)
│   └── sections/           # Page sections (Hero, About, Projects, etc.)
├── public/                 # Static assets
└── README.md
```

## 🛠️ Built With

- **Framework:** Next.js 15.3.4
- **Language:** TypeScript
- **Styling:** Tailwind CSS
- **Animations:** Framer Motion
- **Icons:** Lucide React
- **Deployment:** Vercel

---

⭐ **Star this repository if you found it helpful!**

_Last updated: August 2025_
